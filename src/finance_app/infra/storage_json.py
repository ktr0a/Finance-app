...
from finance_app.core.ports import History, Repository
...
class JsonRepository(Repository):
    ...
    def list_backups(self) -> list[Path]:
        return self._sort_backups_like_core(ascending=False)

    def restore_backup_file(self, path: Path | str) -> bool | None:
        file_path = Path(path)
        try:
        ...
        return True if self._save_like_core(data) is True else False

    def restore_latest_backup(self) -> bool | None:
        backups = self._sort_backups_like_core(ascending=False)
        ...
        restore_status = self.restore_backup_file(newest)
    ...
    def delete_backup_files(self, backups) -> bool:
        cleanup_error = False
    ...
    def clear_undo_stack(self) -> bool:
        if not self.undo_dir.exists():
            return True
        for f in self.undo_dir.glob("*.json"):
            try:
                f.unlink()
            except OSError:
                pass
    ...
    def clear_redo_stack(self) -> bool:
        if not self.redo_dir.exists():
            return True
        for f in self.redo_dir.glob("*.json"):
            try:
                f.unlink()
            except OSError:
                pass

...
from finance_app.core.schema import DATE_FORMAT, TRANSACTION_TYPES
...
def _initdefiners():
    from finance_app.core.core_config import initvars
...
def _name_scoring_calc_plus(text: str) -> float:
    ...
    # Reward word-count sweet spot (typical merchant names are 2GÇô6 words)
    wc = len(tokens)
    if 2 <= wc <= 6:
